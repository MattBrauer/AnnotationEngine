...
# BiocManager::install(c("TissueEnrich",
#                        "AnnotationHub",
...
# devtools::install_github(c("mazetx/VariantEngine",
#                            "mazetx/mazer"))
...
library(ckanr)
library(jsonlite)
#library(TissueEnrich) # bioc; look at Gordon Smyth's reimplemtation of GOseq
library(kableExtra)
library(flextable)
library(edgeR)
...
library(rtracklayer)
library(trackViewer)
library(VariantEngine) # devtools::install_github("mazetx/VariantEngine")
library(mazer) # devtools::install_github("mazetx/mazer")
library(DBI)
...
con <- mazer::athena_connect(catalog_name = "mazeCatalog", json = "auto")
...
ah <- AnnotationHub()
## The following each results in an 'Active external data pointer' that will
## keep the session from being suspended. Either suspend session manually or
## `rm` any of these objects that are loaded.
edb_Hs <- ah[["AH109606"]]
edb_Mm <- ah[["AH109655"]]
txdb_Hs <- ah[["AH111585"]]
txdb_Mm <- ah[["AH75762"]]
...
gene_annotation_base <- annotate_genes(genelist,
...
gene_annotation <- annotate_gene_tissue_rna_specificity(gene_annotation_base)
gene_annotation <- publish_annotation(gene_annotation, ckan_org = new_targets_org, prefix = "01a")
```

```{r tissue-protein}
## 1b. Tissue specificity
gene_annotation <- annotate_gene_tissue_hpa_specificity(gene_annotation_base)
#gene_annotation <- publish_annotation(gene_annotation, ckan_org = new_targets_org, prefix = "01b")
```

```{r kidney-cell-type-expression}
## 2a. Cell type specificity within tissue
gene_annotation <- annotate_gene_cell_type_specificity_kpmp(gene_annotation_base)
gene_annotation <- publish_annotation(gene_annotation, ckan_org = new_targets_org, prefix = "02a")
```

```{r kidney-cell-type-protein}
## 2b. Cell type specificity within tissue
gene_annotation <- annotate_gene_tissue_celltype_hpa_specificity(gene_annotation_base)
gene_annotation <- publish_annotation(gene_annotation, ckan_org = new_targets_org, prefix = "02b")
```
